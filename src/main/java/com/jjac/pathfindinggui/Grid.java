...
public class Grid {
     ...
     * */
    public AdjList<Integer> generateAdjList() {
        AdjList<Integer> adj = new AdjList<>(Integer.class, innerArray.size() * innerArray.get(0).size());
            ...
            for (int x = 0; x < innerArray.get(0).size(); ++x) {
                ...
                if (innerArray.get(y).get(x) > 0) {
                    ...
                    if (x - 1 >= 0 && innerArray.get(y).get(x - 1) != 0)
                        adj.addDirectedEdge(x << 16 | y, (x - 1) << 16 | y,
